...
import { getRiskGradeLabel, getRiskGradeBg, scoreToMarkerColor } from "@/lib/utils";
  ...
  return (
    <View style={styles.container}>
      <View style={styles.mapPlaceholder}>
        ...
        <Text style={styles.modeBadge}>히트맵 {buildings.length}개</Text>
        {showUniversityLayer && (
          <Text style={styles.layerBadge}>
            {Object.values(UNIVERSITY_ZONES)
              .map((z) => `${z.name} ${z.radiusKm}km`)
              .join(" · ")}
          </Text>
        )}
      </View>
      {showHeatmap && (
        <View style={styles.legend}>
          <Text style={styles.legendTitle}>HRI 위험도</Text>
          <View style={styles.legendRow}>
            <View style={[styles.legendDot, { backgroundColor: getRiskGradeBg("safe").border }]} />
            <Text style={styles.legendText}>안전 0~34</Text>
          </View>
          <View style={styles.legendRow}>
            <View style={[styles.legendDot, { backgroundColor: getRiskGradeBg("caution").border }]} />
            <Text style={styles.legendText}>주의 35~64</Text>
          </View>
          <View style={styles.legendRow}>
            <View style={[styles.legendDot, { backgroundColor: getRiskGradeBg("danger").border }]} />
            <Text style={styles.legendText}>위험 65~100</Text>
          </View>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  legendTitle: { fontSize: 11, fontWeight: "700", color: colors.slate[700], marginBottom: 2 },
  legendRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  legendDot: { width: 10, height: 10, borderRadius: 5 },
  legendText: { fontSize: 11, color: colors.slate[600] },
  list: { flex: 1 },
