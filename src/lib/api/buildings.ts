...
import { toAnalysisExplanations, toBuilding, toHRIReport, toRiskGrade } from "./mappers";
...
/** 구·동 단위 검색(예: 중구)을 위해 2글자부터 허용 */
export const MIN_SEARCH_KEYWORD_LENGTH = 2;

/** 백엔드 API는 숫자 buildingId만 허용 */
...
export async function getBuilding(id: string): Promise<Building | null> {
  ...
  if (!isApiBuildingId(id)) {
    return null;
  }

  try {
    const res = await apiClient<BuildingResponse>(`/buildings/${id}`, { auth: false });
...
export async function getHriReportRaw(buildingId: string): Promise<HriReportResponse | null> {
  if (USE_MOCK) return null;

  if (!isApiBuildingId(buildingId)) {
    return null;
  }

  try {
    return await apiClient<HriReportResponse>(`/hri/${buildingId}/report`, { auth: false });
  } catch (e) {
    if (e instanceof ApiError && (e.status === 404 || e.status === 400 || e.status === 500)) {
      return null;
    }
